...
function OrderConfirmation() {
  ...
  // get order and paymentType from location.state if available
  const [order, setOrder] = useState(location.state?.order || null);
  const paymentType = location.state?.paymentType || 'Unknown';

  const [loading, setLoading] = useState(!order); // skip loading if order already passed
    ...
    if (!order) {
      const fetchOrder = async () => {
        try {
          ...
          const res = await axios.get(`http://localhost:8081/api/v1/order-service/order/${orderId}`, {
            headers: {
        ...
        <div className="bg-white p-8 lg:p-12 rounded-2xl shadow-2xl border-t-8 border-green-500">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8 text-left">
            ...
            <div>
              ...
              {/*  use dynamic paymentType */}
              <p className="text-gray-600">Method: <strong>{paymentType}</strong></p>
              <p className="text-gray-600">Status: Paid</p>
              <p className="text-sm text-gray-500 mt-2">
                Tracking ID: <span className="font-mono text-black">TRACK-{order.orderId}</span>
              </p>
            </div>
          </div>
