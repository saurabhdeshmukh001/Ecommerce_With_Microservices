...
function Cart() {
  ...
  const fetchItems = async () => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:8081/api/v1/cart-service/cart/${userId}`,
        {
          headers: {
  ...
  try {
    await axios.patch("http://localhost:8081/api/v1/cart-service/cart-item", {
      cartItemId,
  ...
  const handleRemoveFromCart = async (cartItemIdToRemove) => {
    ...
    try {
      await axios.delete(
        `http://localhost:8081/api/v1/cart-service/cart-item/${cartItemIdToRemove}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
