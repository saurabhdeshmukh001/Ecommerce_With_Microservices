...
function OrderTrackingAdmin() {
  ...
  const fetchUserById = async (userId) => {
    try {
      const res = await axios.get(`http://localhost:8081/api/v1/analytics-service/admin/users/${userId}`, {
  ...
  useEffect(() => {
    const fetchOrdersWithUsers = async () => {
      try {
        const ordersRes = await axios.get("http://localhost:8081/api/v1/analytics-service/admin/orders", {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    try {
      await axios.put(`http://localhost:8081/api/v1/analytics-service/admin/orders/${orderId}`, {
