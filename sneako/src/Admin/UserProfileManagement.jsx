...
const UserProfileManagement = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:8081/api/v1/analytics-service/admin/users", {
        headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:8081/api/v1/analytics-service/admin/users/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
    ...
    <div className="p-4 sm:p-8 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  {["Username", "Email", "Actions"].map((header) => (
                    <th
                      key={header}
        ...
        <div className="mt-6 text-center text-sm text-gray-500">
          Total customer profiles: {users.length}
        </div>
      </div>
    </div>
  );
